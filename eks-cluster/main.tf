...
resource "aws_db_subnet_group" "mlflow" {
  name       = "${local.project_name}-mlflow-dbsubnet"
  subnet_ids = aws_subnet.private_subnet[*].id
  tags = {
    Name = "${local.project_name}-mlflow-dbsubnet"
  }
...
resource "aws_security_group" "rds_pg" {
  ...
  description = "Allow PostgreSQL (5432) from EKS"
  vpc_id      = aws_vpc.vpc.id

  ingress {
    description     = "PostgreSQL from EKS Default SG"
    from_port       = 5432
    to_port         = 5432
    protocol        = "tcp"
  ...
  tags = { Name = "${local.project_name}-rds-sg" }
}


resource "aws_db_instance" "mlflowbackend" {
